...
| **GET/POST**| `/api/devices/config`| Lê ou escreve a configuração remota de um trator (e.g. intervalo de envio, intervalo de leitura do GPS, histórico local, cores do mapa). | `JWT / Secret` |
...
- **Retrocompatibilidade Restrita:** O backend garante *Type Guards*. Ao atualizar a app e pedir novos campos (`localHistoryDays`, `localHistoryMaxGb`, `gpsPollingIntervalMs`), se o `POST/GET` for proveniente de uma build antiga, os *Type Guards* previnem quebras preenchendo automaticamente com *defaults* conservadores (14 dias, 1.0GB, 60s).
