...
- **Lógica de Duplo-Fluxo:** 
  1. **Polling Fixo/Configurável (Antena GPS):** A velocidade a que a antena de GPS acorda é definida pelo utilizador nas configurações (`gps_polling_interval_ms`, default 1 minuto). Este fluxo acorda a antena física para obter uma localização fresca sem necessariamente usar a rede nem gravar, servindo apenas de "base" para detetar movimentos.
  2. **Envio Forçado (Regra da UI):** O intervalo estipulado pelo utilizador nas Configurações (ex: 15 min) (`tracking_interval_ms`). A cada "ping" do GPS (determinado pelo `gps_polling_interval_ms`, ex: 1 min), o serviço cruza a informação com este intervalo. Se o tempo limite esgotar, a localização é gravada/enviada obrigatoriamente, substituindo o algoritmo estacionário do filtro KMP.
